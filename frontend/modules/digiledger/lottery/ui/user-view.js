// 📝 user-view.js (DigiLedger Premium Dashboard - Part 1)
...
window.LotteryUserUI = {
    ...
    mount: function() {
        ...
        workspace.innerHTML = `
            <style>
                ...
                #dl-sidebar { position: fixed; top: 0; left: -280px; width: 260px; height: 100vh; background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-right: 1px solid rgba(255,255,255,0.6); box-shadow: 4px 0 25px rgba(0,0,0,0.05); transition: left 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); z-index: 2000; display: flex; flex-direction: column; }
                #dl-sidebar.active { left: 0; }
                .sidebar-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.3); z-index: 1999; display: none; opacity: 0; transition: opacity 0.3s ease; backdrop-filter: blur(3px); }
                .sidebar-overlay.active { display: block; opacity: 1; }
                #dl-dynamic-view { position: relative; z-index: 1; height: 100%; overflow-y: auto; overflow-x: hidden; padding-bottom: 20px; }
            </style>
            ...
            <div id="dl-sidebar">
                ...
                <div style="padding: 20px; display: flex; flex-direction: column; gap: 15px; font-size: 15px; color: var(--lms-text-dark); flex-grow: 1;">
                    ...
                    <div style="cursor:pointer; padding:10px; border-radius:12px;">⚙️ Settings</div>
    ...
    navigate: function(view) {
        ...
        if (view === 'sales') { window.LotterySalesApp ? window.LotterySalesApp.mount(contentBox) : contentBox.innerHTML = "<h3 style='padding:20px; text-align:center;'>⏳ Loading Sales...</h3>"; } 
            ...
            } else { contentBox.innerHTML = `${this.getTopNavBar("Party Master Error")} <div style="padding:20px; text-align:center; color: #dc3545;"><h3>⚠️ Module Not Found!</h3></div>`; }
        }
        else if (view === 'ledger') { contentBox.innerHTML = `${this.getTopNavBar("Party Ledger")} <div style="padding: 20px;" class="lms_animate_up"><div style="text-align: center; padding: 50px 20px; background: var(--lms-glass-bg); border-radius: 20px; border: 1px solid rgba(255,255,255,0.8);"><h2>📊 Ledger Records Loading...</h2></div></div>`; }
        else if (view === 'stock') { contentBox.innerHTML = `${this.getTopNavBar("Live Stock")} <div style="padding: 20px;" class="lms_animate_up"><div style="text-align: center; padding: 50px 20px; background: var(--lms-glass-bg); border-radius: 20px; border: 1px solid rgba(255,255,255,0.8);"><h2>📦 Inventory Loading...</h2></div></div>`; }
        else if (view === 'report') { contentBox.innerHTML = `${this.getTopNavBar("Day Reports")} <div style="padding: 20px;" class="lms_animate_up"><div style="text-align: center; padding: 50px 20px; background: var(--lms-glass-bg); border-radius: 20px; border: 1px solid rgba(255,255,255,0.8);"><h2>📑 Reports Loading...</h2></div></div>`; }
            ...
            contentBox.innerHTML = `
                <style>
                    ...
                    .lms_summary_card { flex: 1; background: var(--lms-glass-bg); backdrop-filter: blur(16px); border-radius: 16px; padding: 15px; border: 1px solid var(--lms-saffron-border); box-shadow: 0 4px 20px rgba(0,0,0,0.03); position: relative; overflow: hidden; }
                    ...
                    .lms_action_card { background: var(--lms-glass-bg); backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.9); border-radius: 16px; padding: 15px 10px; text-align: center; cursor: pointer; box-shadow: 0 4px 15px rgba(0,0,0,0.02); transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); position: relative; overflow: hidden; }
                    .lms_action_card:hover { transform: scale(1.02) translateY(-3px); box-shadow: 0 12px 25px rgba(0,0,0,0.06); border-color: var(--lms-green-border); }
                    .lms_action_card .icon { font-size: 26px; margin-bottom: 6px; display: block; }
                ...
                <div class="lms_dashboard_inner lms_animate_up">
                    ...
                    <div style="margin-bottom: 20px;">
                        <h1 style="margin: 0; font-size: 20px; font-weight: 800; color: var(--lms-text-dark);"><span style="font-weight:600; font-size:16px; color:var(--lms-text-muted);">${greeting},</span><br>${userName}!</h1>
                    </div>
                    ...
                    <div class="lms_grid">
                        <div class="lms_action_card" onclick="window.LotteryUserUI.navigate('partymaster')">
                            ...
                            <h3 class="title_en">Add Party</h3>
                            <p class="lms_title_local" data-bn="(মহাজন/পার্টি)" data-hi="(पार्टी)">(মহাজন/পার্টি)</p>
                        </div>
                        <div class="lms_action_card" onclick="window.LotteryUserUI.navigate('sales')">
                            <span class="icon">🎟️</span>
                            <h3 class="title_en">Sales Entry</h3>
                            <p class="lms_title_local" data-bn="(খুচরো)" data-hi="(बिक्री)">(খুচরো)</p>
                        </div>
                        <div class="lms_action_card" onclick="window.LotteryUserUI.navigate('dispatch')">
                            <span class="icon">📦</span>
                            <h3 class="title_en">Dispatch</h3>
                            <p class="lms_title_local" data-bn="(স্টক/ডিসপ্যাচ)" data-hi="(डिस्पैच)">(স্টক/ডিসপ্যাচ)</p>
                        </div>
                        <div class="lms_action_card" onclick="window.LotteryUserUI.navigate('stock')">
                            <span class="icon">📊</span>
                            <h3 class="title_en">Live Stock</h3>
                            <p class="lms_title_local" data-bn="(স্টক রিপোর্ট)" data-hi="(स्टॉक रिपोर्ट)">(স্টক রিপোর্ট)</p>
                        </div>
                        <div class="lms_action_card" onclick="window.LotteryUserUI.navigate('report')">
                            <span class="icon">📑</span>
                            <h3 class="title_en">Day Reports</h3>
                            <p class="lms_title_local" data-bn="(দৈনিক রিপোর্ট)" data-hi="(दैनिक रिपोर्ट)">(দৈনিক রিপোর্ট)</p>
                        </div>
                    </div>
                </div>
            `;
